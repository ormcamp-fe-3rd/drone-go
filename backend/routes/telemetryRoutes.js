...
const {
    getTelemetriesByRobotAndOperation,
    getTelemetriesByRobotAndOperationForMap,
 ...
 * @throws {Error} - 서버 에러 발생 시 500 상태 코드와 에러 메시지 반환
 */
router.get('/map', authorizer, async (req, res) => {
    try {
        const { robot, operation } = req.query;
        ...
        const telemetries = await getTelemetriesByRobotAndOperationForMap(robot, operation)
