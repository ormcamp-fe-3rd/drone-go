interface WeatherData {
  ...
  export const fetchWeatherData = async (latitude: number, longitude: number, date: string): Promise<WeatherData | null> => {
    try {
      console.log(`🔍 날씨 데이터 요청: lat=${latitude}, lon=${longitude}, date=${date}`);
  
      const response = await fetch(`${API_BASE_URL}/weather?latitude=${latitude}&longitude=${longitude}&date=${date}`);
